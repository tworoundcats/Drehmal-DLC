...
execute as @s[tag=!finish] run schedule function dlc:modify/helmet/as_b 3.5s
data modify block 1000003 63 1000002 front_text.messages[1] set value '[{"nbt":"ArmorItems[3].tag.display.Name","entity":"@e[type=armor_stand,tag=as,limit=1]","interpret":true},{"text":"⁺ᴀs"}]'
data modify entity @s ArmorItems[3].tag.display.Name set from block 1000003 63 1000002 front_text.messages[1]
item modify entity @s armor.head dlc:helmet/as_modify1
