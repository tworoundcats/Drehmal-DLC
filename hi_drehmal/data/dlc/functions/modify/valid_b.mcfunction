...
execute as @p run tag @s add selector_b
...
execute if score #station int matches 1.. run execute as @p[tag=selector] run function dlc:modify/select_b
...
execute as @p[tag=selector] unless score #station int matches 1.. run tellraw @s ["","[",{"text":"ａｖＳＹＳ","color":"dark_aqua"},"]",{"text":"／／ ＲＥＴＲＩＥＶＥ ＩＴＥＭ ／／","clickEvent":{"action":"run_command","value":"/dlccall execute as @p[tag=selector] run function dlc:modify/retrieve_b"},"hoverEvent":{"action":"show_text","contents":["CLICK TO RETRIEVE ITEM"]}}]
execute as @p[tag=selector] unless score #station int matches 1.. run function dlc:modify/verify_b