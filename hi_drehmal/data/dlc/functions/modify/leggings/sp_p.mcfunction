...
execute as @s[tag=!finish] run schedule function dlc:modify/leggings/sp_p 3.5s
data modify block 1000003 63 1000002 front_text.messages[1] set value '[{"nbt":"ArmorItems[1].tag.display.Name","entity":"@e[type=armor_stand,tag=sp,limit=1]","interpret":true},{"text":"⁺sᴘ"}]'
data modify entity @s ArmorItems[1].tag.display.Name set from block 1000003 63 1000002 front_text.messages[1]
item modify entity @s armor.legs dlc:leggings/sp_modify2
