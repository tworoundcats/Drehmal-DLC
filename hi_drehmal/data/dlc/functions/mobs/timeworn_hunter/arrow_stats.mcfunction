...
execute at @s positioned ~ ~1 ~ run tag @e[type=#arrows,sort=nearest,limit=1,distance=..3,tag=!arrow.inground,tag=!side_arrow_temp,tag=!main_arrow_temp,tag=!special] add hunter_arrow
...
execute at @s rotated ~ 0 run summon arrow ^1 ^ ^ {Tags:["special","hunter_arrow","ground_clear"]}
execute at @s rotated ~ 0 run summon arrow ^-1.25 ^ ^ {Tags:["special","hunter_arrow","ground_clear"]}




execute as @e[type=#arrows,tag=special] run function dlc:mobs/timeworn_hunter/arrow_stats2
