...
execute at @p[tag=shielded] run execute as @e[predicate=entities:hurt2,distance=..16,type=!player,type=!#entities:dummy,type=!#entities:tickless_passive,type=!#entities:highcapacity,type=!#entities:proj] run damage @s[scores={blocking3=..4,blocked=1..}] 1 rubber_nocd by @p[tag=shielded]
