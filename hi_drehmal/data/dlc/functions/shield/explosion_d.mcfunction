execute at @p[tag=shielded] run execute as @e[predicate=entities:hurt2,distance=..16,type=!player,type=!#entities:dummy,type=!#entities:tickless_passive,type=!#entities:highcapacity,type=!#entities:proj] run effect give @s mcdar:stunned 3 1 true
