...
execute as @e[tag=temp_disable2] at @s unless score #low_particles? bool matches 1 run execute at @s run particle lava ~ ~1 ~ 0.1 0.2 0.1 0.2 10 normal
...
execute as @e[tag=temp_disable2] at @s run execute as @e[type=snowball,tag=new,tag=magma_block,sort=nearest,limit=1,tag=!scanned] run function entities:ai/tevus/projectile_big

schedule function entities:ai/tevus/attack2_3 4t
