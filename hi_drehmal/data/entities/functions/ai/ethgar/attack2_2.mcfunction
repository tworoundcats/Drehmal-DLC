effect clear @e[tag=temp_disable] slowness
effect clear @e[tag=temp_disable] glowing
execute as @e[tag=temp_disable] at @s run execute unless score #low_particles? bool matches 1 run execute at @s run particle small_flame ~ ~1 ~ 0.25 1.5 0.25 0.2 30 normal
...
execute as @e[tag=temp_disable] run scoreboard players add @s spm.y 3

function core:rng
scoreboard players operation #rhalon_jump temp = #rand temp
scoreboard players operation #rhalon_jump temp %= #8 const
execute if score #rhalon_jump temp matches 0 run execute as @e[tag=temp_disable] run scoreboard players add @s spm.z 40
execute if score #rhalon_jump temp matches 1 run execute as @e[tag=temp_disable] run scoreboard players add @s spm.x 40
execute if score #rhalon_jump temp matches 2 run execute as @e[tag=temp_disable] run scoreboard players remove @s spm.z 40
execute if score #rhalon_jump temp matches 3 run execute as @e[tag=temp_disable] run scoreboard players remove @s spm.x 40
execute if score #rhalon_jump temp matches 4 run execute as @e[tag=temp_disable] run scoreboard players add @s spm.z 60
execute if score #rhalon_jump temp matches 5 run execute as @e[tag=temp_disable] run scoreboard players add @s spm.x 60
execute if score #rhalon_jump temp matches 6 run execute as @e[tag=temp_disable] run scoreboard players remove @s spm.z 120
execute if score #rhalon_jump temp matches 7 run execute as @e[tag=temp_disable] run scoreboard players remove @s spm.x 120
...
tag @e[tag=temp_disable] remove temp_disable
