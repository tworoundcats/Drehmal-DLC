...
execute if score @s temp matches 0 run tellraw @a[predicate=players:in_generals_arena] ["",{"text":"["},{"text":"Rhalon, the First General", "italic": true, "color": "gray"},{"text":"]"},{"text":" My Radiance… I have… failed you…", "italic": true, "color": "dark_gray"}]
...
execute if score #rhalon_dead bool matches 1 if score #ethgar_dead bool matches 1 if score #ethgar_dead bool matches 1 at @s run function entities:bosses/generals/loot
