...
execute if score #hovadspawned bool matches 1 run function core:scene/hovad/spawn_anim2


execute if score #fightinghovad bool matches 1 run scoreboard players add #tempdeaths timer 1
execute if score #fightinghovad bool matches 1 unless entity @e[type=wither_skeleton,tag=Hovadchear] unless entity @e[type=wither_skeleton,tag=Ultva] run function core:scene/hovad/win2
