...
execute unless score #mb_finish bool matches 1 positioned ~-3 ~-3 ~-3 as @a[dx=5,dy=5,dz=5] unless entity @s[scores={blocking3=..4,blocked=1..}] run damage @s 25 minecraft:khivewrath
execute unless score #mb_finish bool matches 1 positioned ~-3 ~-3 ~-3 as @a[dx=5,dy=5,dz=5] if entity @s[scores={blocking3=..4,blocked=1..}] run function players:items/reticent/main
