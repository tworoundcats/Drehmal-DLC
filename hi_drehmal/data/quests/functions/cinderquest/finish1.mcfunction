...
execute positioned ~ ~ ~ as @p[tag=qst3] at @s run playsound minecraft:dcustom.ui.toast.challenge_complete player @a ~ ~ ~ 1 1
...
execute as @e[name="Cinder",type=villager] run tp @s ~ ~-100 ~
execute as @e[name="Cinder",type=villager] run kill @s

execute positioned ~ ~ ~ as @p[tag=qst3] run advancement grant @s only dlc:cold
execute positioned ~ ~ ~ as @p[tag=qst3] run advancement grant @a only dlc:cold
...
execute unless score #quest3 bool matches 1 run scoreboard players add #quest num 1
execute if score #quest num matches 17 run advancement grant @a only dlc:questmaster
scoreboard players set #quest3 bool 1
kill @e[tag=cinder]
