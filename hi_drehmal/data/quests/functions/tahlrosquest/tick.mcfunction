...
execute if entity @a[distance=21..,tag=temp_interact] as @e[tag=tahlros_i,tag=!qstart,tag=!qend,tag=temp] run function quests:remove_interact

execute at @e[tag=tahlros] as @a[distance=..8,predicate=players:holding/quest/tahlros_like] run function quests:tahlrosquest/check
execute at @e[tag=tahlros,tag=qstart] as @a[distance=..8] unless predicate players:holding/quest/tahlros_like run tag @e[tag=tahlros_i] add disabled
