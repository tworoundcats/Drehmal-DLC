execute unless entity @a[tag=qst9] at @e[tag=chris] run tellraw @a[distance=..12] ["",{"text":"<"},{"text":"Chris","color":"dark_blue"},{"text":"> I'm composing a few songs for an upcoming play, but Mossfield's musical records aren't exactly the most thorough. Not since the fires, at least."}]
