...
execute unless entity @a[tag=qst] run execute at @e[tag=keeko] as @a[distance=..20] run journal quest add @s Keeko "Stablemaster's Struggle"
execute unless entity @a[tag=qst] run execute at @e[tag=keeko] as @a[distance=..20] run journal quest location @s Keeko Drabyel
execute unless entity @a[tag=qst] run execute at @e[tag=keeko] as @a[distance=..20] run journal quest addon @s Keeko [{"text":"One of our horses escaped from the stables, and I can't find him. Would you be able to go track him down and bring him back here for me?"}]
