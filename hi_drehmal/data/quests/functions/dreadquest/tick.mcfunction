...
execute if entity @a[distance=21..,tag=temp_interact] as @e[tag=dread_i,tag=!qstart,tag=!qend,tag=temp] run function quests:remove_interact

execute at @e[tag=dread] as @a[distance=..8,predicate=players:holding/masayoshi] run function quests:dreadquest/check
execute at @e[tag=dread,tag=qstart] as @a[distance=..8,predicate=!players:holding/masayoshi] run tag @e[tag=dread_i] add disabled
