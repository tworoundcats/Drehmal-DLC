...
execute unless entity @a[tag=qst2] at @e[tag=major] as @a[distance=..20] run journal quest add @s Major "Dreamy Dive"
execute unless entity @a[tag=qst2] at @e[tag=major] as @a[distance=..20] run journal quest location @s Major Primal Caverns
execute unless entity @a[tag=qst2] at @e[tag=major] as @a[distance=..20] run journal quest addon @s Major [{"text":"I've been studying this cave, looking for any "},{"text":"artifacts","color":"yellow"},{"text":" blessed by the divine magic in the area. As far as I can tell, there should be "},{"text":"three more","color":"yellow"},{"text":" of them."}]
