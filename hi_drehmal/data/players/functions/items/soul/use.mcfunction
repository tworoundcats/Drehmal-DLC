...
execute at @e[type=minecraft:marker,tag=soul_dmg] run execute as @e[sort=nearest,limit=3,type=!player,type=!#entities:dummy,type=!#entities:highcapacity,type=!#entities:proj,distance=..4] run damage @s[scores={blocking3=..4,blocked=1..}] 5 minecraft:truedamage
