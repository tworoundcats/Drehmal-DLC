...
execute if score #rand temp matches 1 run execute as @e[type=!player,type=!#entities:dummy,type=!#entities:highcapacity,type=!#entities:proj,tag=!secondhit] at @s if predicate entities:hurt run tag @s add secondhit
