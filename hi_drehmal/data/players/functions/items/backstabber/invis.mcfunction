...
execute if score #rand temp matches 1 run execute as @e[tag=!mythic_pvp,type=!#entities:dummy,type=!#entities:highcapacity,type=!#entities:proj,tag=!secondhit] at @s if predicate entities:hurt2 run tag @s add secondhit
