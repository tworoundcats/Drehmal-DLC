...
execute at @e[type=minecraft:marker,tag=mal_dmg] run execute as @e[type=!#entities:fire_mobs,type=!player,type=!#entities:dummy,type=!#entities:highcapacity,type=!#entities:proj,distance=..2] unless entity @s[scores={blocking3=..4,blocked=1..}] run damage @s 21 generic by @p
