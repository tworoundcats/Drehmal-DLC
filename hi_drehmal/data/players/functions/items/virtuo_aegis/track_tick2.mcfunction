...
execute as @e[type=arrow,tag=eyebiter.bit] if score @s eyebiter.id = #id temp store success score #exists temp run tag @s add temp
...
execute if score #exists temp matches 1.. facing entity @e[type=arrow,tag=temp,limit=1] feet positioned as @e[type=arrow,tag=temp,limit=1] run function players:items/virtuo_aegis/track_exists
