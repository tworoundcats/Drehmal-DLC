...
execute if predicate players:holding/frenzy2 at @s[scores={zed=150..}] as @e[tag=!mythic_pvp,predicate=!players:holding/frenzy2,predicate=!entities:invul,type=!#entities:dummy,type=!#entities:highcapacity,type=!#entities:proj,distance=..14] if predicate dlc:25 run function players:items/frenzy/zed
