execute as @s[scores={zenith_cd=80,AvShots=1..}] run playsound minecraft:dcustom.item.lodestone_compass.lock player @s ~ ~ ~ 1 0
...
execute unless predicate players:holding/zenith2 if predicate players:holding/avstate2 run function players:items/avsaber/avdisplay2
...
execute unless predicate players:holding/zenith2 if entity @s[scores={UsedAvSaber=1}] unless predicate players:sneak unless predicate players:holding/avstate at @s unless entity @s[scores={AvShots=1..,AvTimer=..40}] run playsound minecraft:dcustom.entity.generic.extinguish_fire player @s ~ ~ ~ 1 2

execute unless predicate players:holding/zenith2 if entity @s[scores={UsedAvSaber=1}] unless score @s AvTimer matches 41..80 if score @s AvShots matches 1.. at @s anchored eyes positioned ^ ^ ^ positioned ~ ~-0.5 ~ unless predicate players:holding/avstate unless predicate players:sneak run function players:items/avsaber/avfire


execute if predicate players:holding/zenith2 if entity @s[scores={UsedAvSaber=1}] unless predicate players:sneak unless predicate players:holding/avstate2 at @s unless entity @s[scores={AvShots=1..,AvTimer=..40}] run playsound minecraft:dcustom.entity.generic.extinguish_fire player @s ~ ~ ~ 1 2
