...
execute at @s unless entity @e[tag=!mythic_pvp,predicate=!players:holding/first_end,type=!#core:oblivion_immune,distance=..8,nbt={HurtTime:10s},sort=nearest,limit=1] positioned ^ ^ ^3 run function players:items/first_end/use