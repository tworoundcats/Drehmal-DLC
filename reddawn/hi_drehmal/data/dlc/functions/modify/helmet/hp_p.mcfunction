...
execute as @s[tag=!finish] run schedule function dlc:modify/helmet/hp_p 3.5s
data modify block 1000003 63 1000002 front_text.messages[1] set value '[{"nbt":"ArmorItems[3].tag.display.Name","entity":"@e[type=armor_stand,tag=hp,limit=1]","interpret":true},{"text":"⁺ʜᴘ"}]'
data modify entity @s ArmorItems[3].tag.display.Name set from block 1000003 63 1000002 front_text.messages[1]
item modify entity @s armor.head dlc:helmet/hp_modify2
